...
import { ArrowLeft, Save, Trash2, Plus, Zap } from 'lucide-react';
import { apiGetLot, apiUpdateLot, isAdminLoggedIn } from '@/lib/api';
...
export default function AdminLotEditPage() {
  ...
  useEffect(() => {
    ...
    if (existing) {
      ...
      setMinOrder(String(existing.minOrder));
      setMaxOrder(String(existing.maxOrder));
      setStock(String(existing.stock));
      setCooperationLink(existing.cooperationLink);
  ...
  if (!lot) return (
    <div style={{ padding: 40, textAlign: 'center' }}>
      <h2>Lot topilmadi</h2>
      <Link href="/admin/lots"><button className="btn-primary" style={{ marginTop: 16 }}>Orqaga</button></Link>
    </div>
  );
    ...
    <div style={{ display: 'flex', minHeight: '100vh' }}>
      <aside style={{ width: 240, background: 'var(--bg-card)', borderRight: '1px solid var(--border)', padding: '24px 0', display: 'flex', flexDirection: 'column', position: 'sticky', top: 0, height: '100vh' }}>
        ...
        <nav style={{ padding: '16px 12px', flex: 1 }}>
          <Link href="/admin/lots" style={{ textDecoration: 'none' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderRadius: 10, color: 'var(--text-muted)', fontSize: 14, fontWeight: 500 }}>← Lotlar ro'yxati</div>
          </Link>
        </nav>
      </aside>
      ...
      <main style={{ flex: 1, padding: '32px', overflowY: 'auto', maxWidth: 800 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 32 }}>
          ...
          <div>
            <h1 className="font-display" style={{ fontSize: 24, fontWeight: 700 }}>Lotni tahrirlash</h1>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          <div className="card" style={{ padding: 24 }}>
            ...
            <div style={{ display: 'grid', gap: 16 }}>
              ...
              <div>{lbl('Tavsif')}<textarea className="input" value={description} onChange={e => setDescription(e.target.value)} style={{ minHeight: 100, resize: 'vertical' }} /></div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                <div>{lbl('Narxi (UZS)')}<input className="input" type="number" value={price} onChange={e => setPrice(e.target.value)} /></div>
                <div>{lbl('Kategoriya')}<input className="input" value={category} onChange={e => setCategory(e.target.value)} /></div>
              </div>
          ...
          <div className="card" style={{ padding: 24 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              ...
              <button className="btn-outline" style={{ padding: '6px 12px', fontSize: 13 }} onClick={addImage}><Plus size={14} /> Qo'shish</button>
            </div>
            {images.map((img, i) => (
              <div key={i} style={{ display: 'flex', gap: 8, marginBottom: 8, alignItems: 'center' }}>
                ...
                <input className="input" value={img} onChange={e => setImage(i, e.target.value)} placeholder="https://..." />
